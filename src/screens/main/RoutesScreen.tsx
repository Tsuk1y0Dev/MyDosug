...
export const RoutesScreen = () => {
		...
		return [
			{
				...
				totalDuration: schedule.reduce((total, activity) => {
					const start =
						parseInt(activity.startTime.split(":")[0]) * 60 +
						parseInt(activity.startTime.split(":")[1]);
					const end =
						parseInt(activity.endTime.split(":")[0]) * 60 +
	...
	const FavoriteCard = ({
		...
		const handleRemove = () => {
			Alert.alert(removeTitle, `Удалить «${place.name}»?`, [
				{ text: "Отмена", style: "cancel" },
				{
					text: "Удалить",
					style: "destructive",
			...
			<ScrollView style={styles.content}>
				{activeTab === "routes" ? (
					...
					<View style={styles.emptyState}>
						...
						<Text style={styles.emptyStateText}>
							Добавляйте места в избранное на экране поиска; созданные вами
							активности появятся во втором разделе
						</Text>
						<TouchableOpacity
					...
					<View style={styles.favoritesSections}>
						...
						>
							...
							<Text style={styles.createPlaceButtonInlineText}>
								Создать место
							</Text>
						</TouchableOpacity>
