...
type PlannerProviderProps = {
	...
	catalogStandalone?: boolean;
	onDismissCatalogStandalone?: () => void;
};
...
export const PlannerProvider = ({
	...
	onDismissCatalogStandalone,
}: PlannerProviderProps) => {
	const [currentStep, setCurrentStep] = useState(
		catalogStandalone ? 2 : (initialStep ?? 0),
	);
	...
	const [planningRequest, setPlanningRequest] = useState<PlanningRequest>({
		...
		...(catalogStandalone && { activityType: "custom" as const }),
	});
	...
	const removeFromPlan = (activityId: string) => {
		const updatedActivities = currentPlan.activities.filter(
			...
			return {
				...
				travelTimeFromPrevious:
					index === 0
						? 0
						: calculateTravelTime(
								updatedActivities[index - 1].place,
								activity.place,
							),
				order: index,
			};
		...
		const recalculatedActivities = activities.map((activity, index) => {
			...
			return {
				...
				travelTimeFromPrevious:
					index === 0
						? 0
						: calculateTravelTime(activities[index - 1].place, activity.place),
				order: index,
			};
		});
	...
	const dismissCatalogStandalone = useCallback(() => {
		onDismissCatalogStandalone?.();
	}, [onDismissCatalogStandalone]);

	const resetPlanner = () => {
		setCurrentStep(catalogStandalone ? 2 : 1);
		setPlanningRequest({
			...defaultPlanningRequest,
			...(initialTimeSlot && {
				startTime: initialTimeSlot.startTime,
