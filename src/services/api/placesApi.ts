...
const convertPlaceToResponse = (place: Place, id: number): PlaceResponse => {
	return {
		...
		opens_at: place.workingHours.includes("-")
			? place.workingHours.split("-")[0]
			: undefined,
		closes_at: place.workingHours.includes("-")
			? place.workingHours.split("-")[1]
			: undefined,
	...
	async searchPlaces(
		...
		if (data.price_min !== undefined) {
			results = results.filter(
				(p) => (p.avg_price_per_person || 0) >= data.price_min!,
			);
		}
		if (data.price_max !== undefined) {
			results = results.filter(
				(p) => (p.avg_price_per_person || 0) <= data.price_max!,
			);
		}
		if (data.price_min !== undefined) {
			results = results.filter(
