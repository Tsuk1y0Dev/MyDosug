...
export const TimeSelectionStep = () => {
	const { planningRequest, updatePlanningRequest, setCurrentStep } =
		usePlanner();
	...
	const handleStartTimeChange = (event: any, selectedDate?: Date) => {
		if (selectedDate) {
			...
			const currentDuration =
				timeToMinutes(planningRequest.endTime) -
				timeToMinutes(planningRequest.startTime);
			const newEndTime = minutesToTime(
				timeToMinutes(newStartTime) + currentDuration,
			);

			if (!isTimeSlotAvailable(newStartTime, newEndTime)) {
				const availableStart = findNextAvailableSlot(
					newStartTime,
					currentDuration,
				);
				updatePlanningRequest({
					...
					endTime: minutesToTime(
						timeToMinutes(availableStart) + currentDuration,
					),
				});
			} else {
				updatePlanningRequest({
	...
	return (
		<View style={styles.container}>
							...
							return (
								<TouchableOpacity
									...
									style={[
										...
										planningRequest.startTime === slot.start &&
											styles.quickSlotSelected,
									]}
									onPress={() => {
										if (isAvailable) {
											updatePlanningRequest({
						...
						(Platform.OS === "web" ? (
							<input
								...
								onChange={(e) => {
									...
									if (
										!isTimeSlotAvailable(planningRequest.startTime, newEndTime)
									) {
										Alert.alert(
											"Время занято",
											"Это время уже занято другой активностью. Пожалуйста, выберите другое время.",
			...
			<View
				style={[styles.duration, !isSlotAvailable && styles.durationWarning]}
			>
				<Text
					style={[
						styles.durationText,
