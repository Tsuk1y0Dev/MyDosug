...
export const ActivityBlock: React.FC<ActivityBlockProps> = ({
	...
	const findOverlappingActivity = useCallback(
		(newPosition: number): Activity | null => {
			...
			for (const otherActivity of allActivities) {
				...
				if (
					newPosition < otherEnd &&
					newPosition + duration.value > otherStart
				) {
	...
	if (Platform.OS === "web") {
		const handleMoveUp = () => {
			...
			const newPosition = Math.max(0, currentPosition - hourHeight);
	...
	return (
		<PanGestureHandler onGestureEvent={gestureHandler}>
			<AnimatedView style={[styles.activityCard, cardStyle, animatedStyle]}>
				<TouchableOpacity style={styles.content} onPress={handlePress}>
					<View style={styles.header}>
						<View style={styles.dragHandle}>
